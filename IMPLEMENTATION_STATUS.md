...
| **1.1.2** | **REGRESSION ON MAIN — NeoForge currently registers ZERO GameTests.** The 1.1.2 move put the twelve shared tests in `common/`, which cost them NeoForge's `@GameTestHolder`. `GameTestHooks` derives a test's namespace from `getDeclaringClass().getAnnotation(GameTestHolder.class)` and returns **`minecraft`** when it is absent, so every test is namespaced `minecraft` and the `enabledGameTestNamespaces=nexuscore` filter drops all twelve. A second, independent fault was in the same path: the registration shim used `@EventBusSubscriber` without `bus = Bus.MOD`, and `RegisterGameTestsEvent` is an `IModBusEvent`, so the subscriber never fired at all. The bus is fixed; the namespace is not, and cannot be without either a loader annotation in `common/` (which will not compile on the other two) or per-loader holders that DECLARE the test methods and delegate to shared bodies. | **`tools/verify-gametests.sh` exists and is PROVEN — it caught this regression on its first run, against `main`, where `gradlew runGameTestServer` exits 0 having executed nothing.** A run with no count is treated as unmeasured, not passing. Baseline stays **0 / 0 / 0** until the namespace is resolved; the previously reported 12 was NeoForge before the move. Nothing here reaches a manifest until each loader is observed executing a positive count. |
