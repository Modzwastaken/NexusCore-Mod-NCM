...
| **1.1.2** | **In-server test harness — teleport safety, home persistence and the read-only panel now covered.** `runGameTestServer` runs registered GameTests headlessly on NeoForge and exits non-zero on failure; 12 tests across `NexusGameTests` (services) and `NexusWorldGameTests` (world and player). **Still to do:** the three 1.1.1 refusal paths (`/ban`'s propose-time module check needs a safe-mode run configuration the harness does not have; `confirm()`'s spent-token audit and the admin panel's departed-target refusal need a command source and a disconnect), and Fabric/Forge GameTest wiring, which does not exist. | 12 GameTests pass. Proven to fail, each restored byte-identically: `quickMoveStack` delegating to super fails the panel test; `clicked()` delegating to super fails it with the panel's diamonds actually leaving (5 → 0), which is the duplication case itself; `SafeDestination.find` always reporting safe fails both the suffocation and lava tests. |
