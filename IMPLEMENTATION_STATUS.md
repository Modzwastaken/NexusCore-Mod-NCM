...
| **1.1.2** | **12 / 12 / 12 — the shared GameTests execute on all three loaders, observed.** The namespace regression is closed (per-loader holders declaring the methods and delegating to shared bodies; `@GameTestHolder` is what carries a test past `enabledGameTestNamespaces`, and its absence silently namespaced everything `minecraft`). `tools/verify-gametests.sh` refuses a run reporting no count or a count of zero, and `--expect N` refuses a count that drifts down. **Still open:** the three 1.1.1 refusal paths — `/ban`'s propose-time module check needs a safe-mode run configuration, `confirm()`'s spent-token audit and the panel's departed-target refusal need a command source and a disconnect. | **12 executed per loader, verified individually, 0 failed.** The guard is PROVEN BY MUTATION as well as by use: `tools/mutate-gametests.sh` severs `@GameTestHolder` on each loader and asserts the guard goes red — all three detected, every file restored byte-identically. It also fired three times on real regressions, including the one this rung introduced. **The mutation's first version was itself wrong** and reported a false hole in the guard: it cut the explicit `event.register` call, which annotation discovery makes redundant, so 12 tests still ran and the guard correctly passed. The state it claimed to create did not exist — recorded because it is the same mistake this tooling exists to catch, made inside the tool built to catch it. |
