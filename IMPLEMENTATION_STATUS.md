...
| **1.1.2** | GameTests: teleport safety, home/warp persistence, punishment enforcement, permission gating. Plus the missing `AdminMenu` read-only container test. **Also the two 1.1.1 refusal paths that no unit test can reach**: `/ban`'s propose-time module check and `confirm()`'s spent-token audit, and the admin panel's re-resolve-or-refuse on a target who logged out — all three are command/GUI call sites needing a `CommandSourceStack` or a live player. | The four GameTests named in M5 exist and pass; a test proves items cannot enter or leave the admin menu; and each of the three refusal paths above is driven end to end, so removing its guard fails a test rather than passing silently. |
